  ...
  Mutation: {
    async createAnimal(parent, { name, type, intake }, { models }) {
      ...
      return models.Animal.create({
        ...
        .catch((err) => { throw new ApolloError(err) })
        .then(async (data) => {
          await writeLog(models, `Created animal "${name}"`, 'animal')
          return data
            .populate('habitat')
            .populate('type')
            .execPopulate()
    ...
    async updateAnimal(parent, { _id, ...args }, { models }) {
      ...
      return models.Animal.findByIdAndUpdate(_id, args, { new: true })
        ...
        .then(async (data) => {
          ...
          if (!(Object.keys(args).length === 1 && args.habitat)) await writeLog(models, `Updated animal "${data.name}"`, 'animal')
          return data
            .populate('habitat')
            .populate('type')
            .execPopulate()
    ...
    async deleteAnimal(parent, { _id }, { models }) {
      return models.Animal.findByIdAndDelete(_id)
        .catch((err) => { throw new ApolloError(err) })
        .then(async (data) => {
          await writeLog(models, `Deleted animal "${data.name}"`, 'animal')
          return data
        })
    }
  }
